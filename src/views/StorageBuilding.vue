<template>
  <div class="relative w-full h-full">
    ...
    <div v-if="submenus.truss" class="scroll-smooth overflow-hidden flex py-3 absolute left-0 z-50 bg-white/50 max-w-1/4 w-1/4 mr-auto animate-top" :class="!fullscreen ? 'animate-top-large' : 'animate-top-small'">
      <div class="relative flex flex-col scroll-smooth w-full items-start justify-start py-3 pl-14 min-h-[100vh]">
        <div v-for="(truss, index) in trussOptions" :key="index" @click="toggleTruss(truss)" class="flex flex-col cursor-pointer text-center justify-start items-start mb-7" @mouseover="trussHoverBox[index] = true" @mouseleave="trussHoverBox[index] = false">
          <img :src="truss === 'Single Truss' ? './models/truss/single-truss/single-truss.jpg' : './models/truss/double-truss/double-truss.jpg'" :alt="truss" class="w-48 h-48 rounded-md border-4 object-contain transition" :class="trussHoverBox[index] || (truss === 'Single Truss' && selectedTruss === 'Single Truss') || (truss === 'Double Truss' && selectedTruss === 'Double Truss') ? 'border-primary' : 'border-gray-300'">
    ...
    <div v-if="submenus.len" class="scroll-smooth overflow-hidden flex flex-col py-3 absolute inset-x-0 z-50 bg-white colors-container pl-14 mr-auto animate-top" :class="!fullscreen ? 'animate-top-large' : 'animate-top-small'">
    ...
    <div v-if="submenus.wid" class="scroll-smooth overflow-hidden flex flex-col py-3 absolute inset-x-0 z-50 bg-white colors-container pl-14 mr-auto animate-top" :class="!fullscreen ? 'animate-top-large' : 'animate-top-small'">
      <div class="flex items-center">
        <primary-button v-for="(wid, index) in widths" :key="wid" :text="wid + ' meters'" @click="dimensionChange(wid, 'wid')" :active="wid === width" :class="index === 0 && 'ml-0'" />
      </div>
      <div class="block w-full mt-2 ml-px">
    ...
    <div v-if="submenus.hei" class="scroll-smooth overflow-hidden flex py-3 absolute inset-x-0 z-50 bg-white colors-container pl-14 mr-auto animate-top" :class="!fullscreen ? 'animate-top-large' : 'animate-top-small'">
      <primary-button v-for="(hei, index) in heights" :key="hei" :text="hei + ' meters'" @click="dimensionChange(hei, 'hei')" :active="hei === height" :class="index === 0 && 'ml-0'" />
    </div>
